...
const Accueil = () => {
  ...
  return (
    <View>
      <Button title="N'afficher que les tracks artiste" onPress={() => { setFiltred(!filtred) }}></Button>
      {list.length > 0 ? <FlatList
        ...
        renderItem={({ item }) => (
          <View>
            <Pressable onPress={() => {
              navigation.navigate("Les détails", { name: item.data.artiste, genre: item.data.genre, price: item.data.price, image: item.data.image, resume: item.data.resume, artiste: item.data.artiste, add: false });
            }}>
